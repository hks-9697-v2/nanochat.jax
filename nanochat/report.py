...
def get_gpu_info():
    """Get accelerator information via JAX."""
    devices = jax.devices()
    # Filter to non-CPU devices (GPU/TPU)
    ...
    info = {
        ...
        "names": [d.device_kind for d in accelerators],
        "platform": accelerators[0].platform,  # "gpu" or "tpu"
    }
...
def estimate_cost(gpu_info, runtime_hours=None):
    ...
    # Try to identify accelerator type from name
    hourly_rate = None
    device_name = gpu_info["names"][0] if gpu_info["names"] else "unknown"
    for device_type, rate in gpu_hourly_rates.items():
...
"""
    ...
    if gpu_info.get("available"):
        device_names = ", ".join(set(gpu_info["names"]))
        platform = gpu_info.get("platform", "unknown")
        header += f"""- Accelerators: {gpu_info['count']}x {device_names} ({platform})
"""
    else:
...
def get_report():
    # just for convenience, only process 0 logs to report
